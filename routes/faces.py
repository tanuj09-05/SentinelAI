...
def faces_page():
    ...
    try:
        with sqlite3.connect(DATABASE_PATH, timeout=5) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, name, photo_path, created_at FROM faces WHERE user_id = ? ORDER BY created_at DESC",
                (user_id,),
            )
...
def upload_face():
    ...
    if not fm:
        os.remove(filepath)
        return (
            jsonify({"success": False, "message": "Face Manager is not initialized."}),
            500,
        )
...
def delete_face(face_id):
    ...
    try:
        with sqlite3.connect(DATABASE_PATH, timeout=5) as conn:
            ...
            cursor.execute(
                "SELECT photo_path FROM faces WHERE id = ? AND user_id = ?",
                (face_id, user_id),
            )
            row = cursor.fetchone()
            ...
            fm = get_face_manager()
            if fm:
                fm.load_database()  # Reload known faces
